...
void    HC_SR04::setOffsetLeft(void) {
    ...
    while (counter < 250) {
        unsigned long cycle_time = millis() + 5; // 5ms per measurement cycle
...
uint16_t HC_SR04::getDistanceSensorLeft(void) {
    ...
    echo_counter_local = _echo_counter;
    _echo_counter      = 0;
    ...
    distance_local  = _echo_counter - echo_counter_local;
